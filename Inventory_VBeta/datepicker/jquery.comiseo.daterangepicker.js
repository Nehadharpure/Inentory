	...
	$.widget('comiseo.daterangepicker', {
		...
		options: {
			...
			presetRanges: [
						...
						{text: 'Year Ago', dateStart: function() { return moment().subtract('year', 1).startOf('year') }, dateEnd: function() { return moment().subtract('year', 1).endOf('year') } }
						],
			verticalOffset: 0,
   ...
   function showd()
	{
		...
		var year = $('#example').val();		
		var startyear = year.substr(10,10);
		var endyear = year.substr(29,10);
		var startyearY = year.substr(10,4);	
		var store = $('#store').val();
		...
		$.ajax({ 
						type: "POST",
						url: 'ajax/ajax_start_to_end_date.php',
						data: { 'startyear':startyear, 'endyear':endyear, 'store':store, 'startyearY':startyearY},
		...
		var menuItem = $('#MenuItem').val();
		  $.ajax({ 
						type: "POST",
						url: 'ajax/ajax_product_dashboard.php',
						data: { 'startyear':startyear, 'endyear':endyear, 'store':store,'menu':menu,'menuItem':menuItem},
						success: function(data){
							$('#allchanges').html(data);
					},
		});	
		}
		else if(formname == 'analysis2')
		{
			$('#target').addClass('loading');	
			var menu = $('#menu').val();
		  $.ajax({ 
						type: "POST",
						url: 'ajax/ajax_analysis2.php',
						data: { 'startyear':startyear, 'endyear':endyear, 'store':store,'menu':menu},
						success: function(data){
							$('#salestrend').html(data);
					},
		});		
		}
		else if(formname == 'analysis3')
		{
			$('#target').addClass('loading');	
			var menu = $('#menu').val();
		  $.ajax({ 
						type: "POST",
						url: 'ajax/ajax_analysis3.php',
						data: { 'startyear':startyear, 'endyear':endyear, 'store':store,'menu':menu},
						success: function(data){
							$('#salestrend').html(data);
					},
		...
		else if(formname == 'analysis4')
		{
			$('#target').addClass('loading');	
		  $.ajax({ 
						type: "POST",
						url: 'ajax/ajax_analysis4.php',
						data: { 'startyear':startyear, 'endyear':endyear, 'store':store},
						success: function(data){
							$('#salestrend').html(data);
					},
		});		
		}
		else if(formname =='analysis8')
		{
			$('#target').addClass('loading');	
				var menu = $('#menu').val();
			var menuItem = $('#MenuItem').val();
		  $.ajax({ 
						type: "POST",
						url: 'ajax/ajax_analysis8.php',
						data: {'startyear':startyear, 'endyear':endyear, 'store':store, 'startyearY':startyearY,'menu':menu,'menuItem':menuItem},
						success: function(data){
	...
	function buildPresetsMenu(classnameContext, options, onClick) {
		...
		function init() {
			...
			$.each(options.presetRanges, function() {
				$('<li><a href="#" onclick="">' + this.text + '</a></li>')
